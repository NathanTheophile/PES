...
namespace TacticalPort.EditorTools
{
    internal static class Creator_FileSaver
    {
        ...
        public static void SetString(SerializedObject pObject, string pPropertyName, string pValue) =>
            pObject.FindProperty(pPropertyName).stringValue = pValue ?? string.Empty;

        public static void SetInt(SerializedObject pObject, string pPropertyName, int pValue) =>
            pObject.FindProperty(pPropertyName).intValue = pValue;

        public static void SetBool(SerializedObject pObject, string pPropertyName, bool pValue) =>
            pObject.FindProperty(pPropertyName).boolValue = pValue;

        public static void SetEnum(SerializedObject pObject, string pPropertyName, int pValue) =>
            pObject.FindProperty(pPropertyName).enumValueIndex = pValue;

        public static void SetColor(SerializedObject pObject, string pPropertyName, Color pValue) =>
            pObject.FindProperty(pPropertyName).colorValue = pValue;

        public static void SetObject(SerializedObject pObject, string pPropertyName, Object pValue) =>
            pObject.FindProperty(pPropertyName).objectReferenceValue = pValue;
        ...
        public static void DrawSectionHeader(string pLabel)
        {
            GUILayout.Space(6f);
            EditorGUILayout.LabelField(pLabel, EditorStyles.boldLabel);
        }
