...
| Meta/Progression | Sauvegarde/config | `One Piece Tactics/save.gd`, `SaveConfigs.gd` | `PES_Unity/Assets/_Project/Infrastructure/Serialization/*` | In Progress | Couvrir la migration de schéma (v2+) et brancher un store persistant runtime (PlayerPrefs/fichier) | Contrat de save minimal versionné + sérialiseur tolérant aux versions invalides + store de référence testable en mémoire | Medium | `SessionSaveSerializerTests` + `ProductFlowControllerTests` | Partial | Infra | Sprint H |
