...
| Skills | Système de skills / effets | `One Piece Tactics/skills/SkillScript.gd`, `skills/SkillEffectTemplate.gd` | `PES_Unity/Assets/_Project/Combat/Actions/CastSkillAction.cs` + `Targeting/SkillTargetingService.cs` + `Resolution/SkillResolutionService.cs` | In Progress | Finaliser catalogue multi-effets avancé (buffs/debuffs) + brancher raycast physique Unity (AOE splash mono-cible disponible, coûts/cooldowns + tick de cooldown gérés) | Ciblage x/z + LOS raycast domaine + bonus portée par élévation + coût/cooldown testés | High | `CastSkillActionTests` + `VerticalSliceBattleLoopTests` + CI Unity EditMode | Partial | Gameplay | Sprint G |
...
| Units/Stats | Socle caractéristiques RPG (AP/MP/PO/élévation/invocs/HP/assiduité/rapidité + attaques/puissances/défenses/résistances par élément + critiques) | (N/A explicite côté scripts listés) | `PES_Unity/Assets/_Project/Core/Simulation/CombatantRpgStats.cs` + `DamageFormulaCalculator.cs` + `Presentation/Configuration/EntityArchetypeAsset.cs` | In Progress | Brancher ces stats dans toutes les résolutions d'actions (attaque de base + skills) et dans l'initiative/tour | Modèle domaine des stats + formule de dégâts déterministe + chargement data-driven via archetypes | High | `DamageFormulaCalculatorTests` + `BattleState`/`ActionResolver` tests | Partial | Core | Sprint G |
| States | Machine d'états combat | `One Piece Tactics/states/StateScript.gd`, `StatePreview.gd` | `PES_Unity/Assets/_Project/Core/Simulation/BattleState.cs` + `Core/TurnSystem/*` | In Progress | Étendre au catalogue complet d'états Godot (buff/debuff/interruptions) et au preview UI | Fondations status-effects déterministes (Poison, durée, tick début/fin de tour configurable, snapshot/replay) | High | `StatusEffectStateTests` + `VerticalSliceBattleLoopTests` | Partial | Core | Sprint G |
