...
| Skills | Système de skills / effets | `One Piece Tactics/skills/SkillScript.gd`, `skills/SkillEffectTemplate.gd` | `PES_Unity/Assets/_Project/Combat/Actions/CastSkillAction.cs` + `Targeting/SkillTargetingService.cs` + `Resolution/SkillResolutionService.cs` | In Progress | Finaliser multi-effets (AOE, buffs/debuffs) + brancher raycast physique Unity (coûts ressources/cooldowns désormais gérés côté domaine) | Ciblage x/z + LOS raycast domaine + bonus portée par élévation + coût/cooldown testés | High | `CastSkillActionTests` + CI Unity EditMode | Partial | Gameplay | Sprint G |
