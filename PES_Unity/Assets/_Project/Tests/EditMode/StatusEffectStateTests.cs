...
namespace PES.Tests.EditMode
{
    public sealed class StatusEffectStateTests
    {
        ...
        public void VulnerableStatus_AmplifiesIncomingDamage_AndExpiresOnConfiguredTick()
        {
            ...
            state.SetEntityHitPoints(actor, 20);
            state.SetStatusEffect(actor, StatusEffectType.Vulnerable, remainingTurns: 1, tickMoment: StatusEffectTickMoment.TurnStart);

            var applied = state.TryApplyDamage(actor, 10);
            Assert.That(applied, Is.True);
            Assert.That(state.TryGetEntityHitPoints(actor, out var hpAfterAmp), Is.True);
            Assert.That(hpAfterAmp, Is.EqualTo(10));
        ...
        [Test]
        public void InvulnerableStatus_BlocksIncomingDamage_AndExpiresOnConfiguredTick()
        {
            var state = new BattleState();
            var actor = new EntityId(903);
            state.SetEntityHitPoints(actor, 20);
            state.SetStatusEffect(actor, StatusEffectType.Invulnerable, remainingTurns: 1, tickMoment: StatusEffectTickMoment.TurnStart);

            var applied = state.TryApplyDamage(actor, 10);
            Assert.That(applied, Is.False);
            Assert.That(state.TryGetEntityHitPoints(actor, out var hpBlocked), Is.True);
            Assert.That(hpBlocked, Is.EqualTo(20));

            var tickDamage = state.TickStatusEffects(actor, StatusEffectTickMoment.TurnStart);
            Assert.That(tickDamage, Is.EqualTo(0));
            Assert.That(state.GetStatusEffectRemaining(actor, StatusEffectType.Invulnerable), Is.EqualTo(0));
        }

        [Test]
        public void ElementWheel_WhenDefenderIsWeakToAttackElement_Applies25PercentBonusDamage()
        {
            var state = new BattleState();
            var actor = new EntityId(904);
            state.SetEntityHitPoints(actor, 40);
            state.SetEntityStat(actor, EntityStatType.AffinityElement, (int)AttackElement.Physique);

            var applied = state.TryApplyDamage(actor, 20, AttackElement.Contondante);
        ...
        [Test]
        public void ElementWheel_WhenDefenderIsNotWeakToAttackElement_AppliesBaseDamage()
        {
            var state = new BattleState();
            var actor = new EntityId(905);
            state.SetEntityHitPoints(actor, 40);
            state.SetEntityStat(actor, EntityStatType.AffinityElement, (int)AttackElement.Physique);

            var applied = state.TryApplyDamage(actor, 20, AttackElement.Elementaire);
